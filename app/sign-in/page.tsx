...
function AuthFrame({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) { return <main className="flex min-h-screen items-center justify-center bg-[#faf8f6] px-5"><section className="w-full max-w-md rounded-3xl bg-white p-8 shadow-xl shadow-[#552c2012] ring-1 ring-[#eadfd8]"><div className="mb-8 text-center"><div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-2xl bg-[#9f1d35] text-xl text-white">♥</div><h1 className="font-serif text-3xl text-[#392b27]">{title}</h1><p className="mt-2 text-sm text-[#9f8a7d]">{subtitle}</p></div>{children}</section></main> }
